...
public class ConditionRouter implements Router, Comparable<Router> {
    ...
    /**
     * 对路由规则做预处理，然后调用 parseRule 方法分别对服务提供者和消费者规则进行解析，最后将解析结果赋值给 whenCondition 和 thenCondition 成员变量。
     * @param url
     */
    ...
    /**
     * 先是调用 matchWhen 对服务消费者进行匹配，如果匹配失败，直接返回 Invoker 列表。如果匹配成功，再对服务提供者进行匹配，匹配逻辑封装在了 matchThen 方法中。
     * @param invokers
     * @param url        refer url
     * @param invocation
     * @param <T>
    ...
    boolean matchWhen(URL url, Invocation invocation) {
        ...
        return whenCondition == null || whenCondition.isEmpty() || matchCondition(whenCondition, url, null, invocation);// 进行条件匹配
    }

    private boolean matchThen(URL url, URL param) {
        // 服务提供者条件为 null 或空，表示禁用服务
        return !(thenCondition == null || thenCondition.isEmpty()) && matchCondition(thenCondition, url, param, null);// 进行条件匹配
    }
    ...
    private static final class MatchPair {
        ...
        private boolean isMatch(String value, URL param) {
            ...
            // 情况四：matches 和 mismatches 均为空，此时返回 falseisMatchGlobPattern
            return false;
        }
    }
}